## Hi there 👋

# Conner Gardiner

I'm a student at Brigham Young University-Idaho interested in Network Engineering, Data Science, and Machine Learning. Concurrently, I am working as a Jr Network Engineer within the field which has resulted in obtaining hands-on experience, obtaining industry certfications and developing personal projects to showcase applicable skillsets for the ever changing service requirements.

## About Me

- I’m currently learning how to interact with, manage, and properly scale an AWS VM for a personal project. In addition, seeking to fine tune my ability to use Linux and its related sub-platforms as this is just something of interest for open source software.
- I’m looking for help with organizing a collection of various coding/cyber bootcamp turtorials to streamline a training self-paced training algorithm! The intent for this is to streamline the practice of learning, from start to finish, as implemented research methodologies for improving the process are on a constant rotation to see what sticks best for gearing up myself and others within the field for the day to day impact of the industry.
- Ask me about my Senior Project, still in the predev phase, however, excited to showcase in the coming months to a general pre-release audience!


## Technologies and Tools

- Proficient in: Python, SQL, C#, Linux, R, and various industry networking technologies.


## Let's Connect!

- 📫 Here is my Linkedin Profile https://www.linkedin.com/in/conner-gardiner/  

Feel free to explore my repositories and don't hesitate to reach out; enjoy the process of the journey!


